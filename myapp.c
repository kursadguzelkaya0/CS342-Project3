...
void *threadfunc1(void *a) {
    ...
    setarray(request1, NUMR, 5,5,5,1,2);
    pr(tid, "REQ", NUMR, request1);
    rm_request(request1);
    sleep(4);
    setarray(request2, NUMR, 3,3,2,4,4);
...
void *threadfunc2 (void *a) {
    ...
    setarray(claim, NUMR, 8,8,8,8,8);
    rm_claim(claim);
    setarray(request1, NUMR, 2,3,3,1,5);
    pr(tid, "REQ", NUMR, request1);
    rm_request(request1);
    sleep(2);

    setarray(request2, NUMR, 4,4,2,2,3);
    pr(tid, "REQ", NUMR, request2);
    rm_request(request2);
    rm_release(request1);
    rm_release(request2);
...
int main(int argc, char **argv) {
    ...
    tids[i] = i;
    pthread_create(&(threadArray[i]), NULL, (void *) threadfunc1, (void*) &tids[i]);
